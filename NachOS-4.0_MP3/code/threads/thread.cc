...
void Thread::Yield()
{
    ...
    /*周改的*/
    int ExecutionTime = this->GetExecutionTime();
    int ApproximateBurstTime = this->GetApproximateBurstTime();

    this->SetApproximateBurstTime(max(0, ApproximateBurstTime - ExecutionTime));
    /*周改的*/
    nextThread = kernel->scheduler->FindNextToRun();
    if (nextThread != NULL)
    {
        kernel->scheduler->ReadyToRun(this);
        nextThread->SetWaitingTime(0); //周改的
...
void Thread::Sleep(bool finishing)
{
    ...
    /*周改的*/
    int originalApproximateBurstTime = this->GetApproximateBurstTime();
    int executiontime = this->GetExecutionTime();
    int newApproximateBurstTime = originalApproximateBurstTime * 0.5 + executiontime * 0.5;
    this->SetApproximateBurstTime(newApproximateBurstTime);
    DEBUG(dbgList, "[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID() << "] update approximate burst time, from: [" << originalApproximateBurstTime << "], add [" << executiontime << "], to [" << newApproximateBurstTime << "]");
    /*周改的*/
