...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (strcmp(argv[i], "-ep") == 0)
        {
            ...
            execfilePriority[execfileNum] = execfilePriority[execfileNum] > 149 ? 149 : execfilePriority[execfileNum];
            execfilePriority[execfileNum] = execfilePriority[execfileNum] < 0 ? 0 : execfilePriority[execfileNum];
            //cout << "File name is: " << execfile[execfileNum] << " and its priority number is " << execfilePriority[execfileNum];
        }
        /*周改的*/
    }
}
...
#endif // FILESYS_STUB
    //postOfficeIn = new PostOfficeInput(10); //周改的，根據instruction
    //postOfficeOut = new PostOfficeOutput(reliability); //周改的，根據instruction
...
Kernel::~Kernel()
{
    ...
    delete synchDisk;
    delete fileSystem;
    //delete postOfficeIn; //周改的，根據instruction
    //delete postOfficeOut; //周改的，根據instruction
...
void Kernel::ExecAll()
{
    ...
    {
        int a = Exec(execfile[i], execfilePriority[i]); //周改的
    }
    currentThread->Finish();
    //Kernel::Exec();
}

int Kernel::Exec(char *name, int priorty)
{
    t[threadNum] = new Thread(name, threadNum);
    t[threadNum]->space = new AddrSpace(usedPhyMem); //有改 有改 有改
    /*周改的*/
    t[threadNum]->SetPriority(priorty);
    t[threadNum]->SetApproximateBurstTime(0);
