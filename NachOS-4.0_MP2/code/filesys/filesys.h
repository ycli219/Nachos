...
public:
    ...
    int WriteFile(char *buffer, int size, OpenFileId id) // 李彥璋改的
    {
    ...
    int ReadFile(char *buffer, int size, OpenFileId id) // 李彥璋改的
    {
        OpenFile *openfile = OpenFileTable[id];
        if (openfile == NULL)
            return -1;
        int NumRead = openfile->Read(buffer, size);
        return NumRead;
    }
    int CloseFile(OpenFileId id) // 李彥璋改的
    {
        OpenFile *openfile = OpenFileTable[id];
        if (openfile == NULL)
            return -1;
