...
bool FileSystem::Create(char *name, int initialSize)
{
    ...
    OpenFile *open_tool = directoryFile;
    OpenFile *belong_dir = open_tool;
    ...
    while(token != NULL) {
        ...
        belong_dir = open_tool;
        open_tool = new OpenFile(sector);
        directory->FetchFrom(open_tool);
    ...
    if(token == NULL) {
        token = pre_token;
        directory->FetchFrom(belong_dir);
    }
...
OpenFile * FileSystem::Open(char *name)
{
    ...
    while(token != NULL) {
        if( (sector = directory->Find(token) ) ==-1  || directory->checkIfDir(token) == FALSE ) {
            break;
        }
        //sector = directory->Find(token);
        open_tool = new OpenFile(sector);
...
bool FileSystem::Remove(char *name, bool doRecursive)
{
    Directory *directory;
    PersistentBitmap *freeMap;
    FileHeader *fileHdr;
...
//有改
void FileSystem::List(char *name, bool doRecursive)
{
    Directory *directory = new Directory(NumDirEntries);
    directory->FetchFrom(directoryFile);
