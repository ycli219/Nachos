...
static void Copy(char *from, char *to)
{
    ...
    openFile = kernel->fileSystem->Open(copied); //有改
...
{
    // MP4 Assignment
    kernel->fileSystem->CreateDir(name); //有改
}
...
int main(int argc, char **argv)
{
    ...
    {
        kernel->fileSystem->Remove(removeFileName, recursiveRemoveFlag); //有改
    }
    ...
    if (dirListFlag)
    {
        kernel->fileSystem->List(listDirectoryName, recursiveListFlag); //有改
    }
    if (mkdirFlag)
    {
        // MP4 mod tag
